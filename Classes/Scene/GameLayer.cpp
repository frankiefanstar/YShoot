...
void GameLayer::initMenu() {
	...
	CCMenuItemFont* pause = CCMenuItemFont::create("Pause", this, menu_selector(GameLayer::doPause));
	pause->setAnchorPoint(ccp(0, 0));
	pause->setPosition(ccp((winSize.width - pause->getContentSize().width),(winSize.height/2 + 100)));

	CCMenuItemFont* resume = CCMenuItemFont::create("Resume", this, menu_selector(GameLayer::doResume));
	resume->setAnchorPoint(ccp(0, 0));
	resume->setPosition(ccp((winSize.width - resume->getContentSize().width),(winSize.height/2 )));

	CCMenuItemFont* back = CCMenuItemFont::create("Back", this, menu_selector(GameLayer::doBackHome));
	back->setAnchorPoint(ccp(0, 0));
	back->setPosition(ccp((winSize.width - back->getContentSize().width),(winSize.height/2 - 100 )));
	CCMenu* gameMenu = CCMenu::create(pause, resume ,back,
				NULL);
...
//            selChild->update(dt);  //根据HP 检测自己是否active
            ...
            if (( tag == TAG_ENEMY_BULLET) || (tag == TAG_HERO_BULLET )|| (tag == TAG_ENEMY)) {
            	//如果生命值小于0 则销毁
